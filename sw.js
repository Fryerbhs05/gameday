...
const CACHE_VERSION = 'conflicted-v88'; // v88: Exposure control placement revised (Matt, 2026-07-19). The position pills move UP into the shared period bar's .pb-seg-row — the slot Leaders uses for CARRIED/CRUSHED and Snapshot for List/Grid — so they sit inside the one consolidated controls frame and stay visible at all times. My rosters / Opp rosters goes back into the Filters bottom sheet alongside axis and roster scope, so the sheet is again the single home for every hideable Exposure control. Because mode is no longer on screen, the active position pill tints with it (coral = my rosters, navy = opp rosters) as the standing visual cue for which mode is live. Builds on v87's grid removal and period-bar normalization.
