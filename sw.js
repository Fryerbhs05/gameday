...
const CACHE_VERSION = 'conflicted-v87'; // v87: Exposure map normalized onto the shared #cf-period-bar (Matt, 2026-07-19). The grid/heatmap view is REMOVED entirely on every breakpoint (it was already display:none on mobile) — the leaderboard is now Exposure's only presentation. Exposure no longer builds its own #exp-week-select/#exp-season-select or its own Filters button; week/season/Filters come from the one shared bar, and the My-rosters/Opp-rosters mode switch rides that bar's .pb-seg-row (bar gains .pb-exposure) exactly like Leaders' CARRIED/CRUSHED and Snapshot's List/Grid. Axis + roster scope stay in the Filters bottom sheet (mode was removed from the sheet so there is one source of truth); the position pills stay permanently visible below the bar in a navy-300 framed strip that scrolls horizontally on narrow screens. Groundwork for making league show/hide a global filter across GameDay, Snapshot, Leaders and Exposure.
