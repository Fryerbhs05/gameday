...
const CACHE_VERSION = 'conflicted-v91'; // v91: League show/hide is now GLOBAL (Matt, 2026-07-19). One hiddenLeagues set drives GameDay, Snapshot, Leaders and Exposure via a new shared cfRenderLeagueSection() partial injected into all three filter sheets (the drawer, Exposure's settings sheet, Leaders' sheet) — previously the toggles only hid Loyalties player cards and Snapshot rows. Scoreboard tiles now drop hidden leagues, and the export 'Winning N of X' hero counts the same filtered set, so hiding the league you got shellacked in actually removes it from the shared card (both N and X). Exposure filters via visibleLeagues() on my/against/bench lists so counts, KPIs and the leaderboard all reflect it; Leaders filters at RENDER time (not extractRows) so the per-week localStorage cache stays complete and toggling back on never refetches — filtering before the gold pass also correctly drops a tick when the conflict only existed in a hidden league. Because the count chip was retired in v90, the Filters button now takes a coral .is-filtering state whenever anything is hidden (cfSyncFilterActive) — the only standing cue that aggregates are filtered — plus a 'Show all leagues' reset inside the section. Watch Priority deliberately excluded. Still session-only: resets on reload, per the filter-persistence rule.
