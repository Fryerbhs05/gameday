...
const CACHE_VERSION = 'conflicted-v89'; // v89: Exposure gets a page header above the period bar (Matt, 2026-07-19), matching Watch Priority's .wp-head stacking. New optional .pb-view-head block at the top of #cf-period-bar's inner wrapper carries kicker / title / subtitle and only renders for views that opt in via a bar class (.pb-exposure today), so GameDay / Snapshot / Leaders are untouched. #view-exposure now holds no chrome at all — title, week/season, Filters and position pills all live in the one shared bar — so its top padding drops to 0. Subtitle is now mode-aware ('your rosters' vs "your opponents' rosters"): with the mode switch moved into the Filters sheet in v88, that line is the only on-screen indicator of whose rosters are showing. Also retires stale copy referencing the removed grid cells and the old Positive/Negative labels.
